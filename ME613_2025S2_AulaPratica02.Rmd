...
```{r}
dados = read_delim("handheight.txt", show_col_types = FALSE)
head(dados)
#Palmo da mão
x =  dados$HandSpan 
#Altura
...
# Criando um  gráfico de dispersão para poder visualizar melhor  o tamanho do palmo da mão entre a idade (variável preditora X) e a altura (variável resposta Y)

 ggplot(dados, aes(x = dados$HandSpan, y = dados$Height)) +
  geom_point(color = "blue", size = 2) +
  labs(title = "Gráfico de Dispersão: Palmo da Mão x Altura",
       x = "Palmo da Mão",
       y = "Altura  ") +
  theme_minimal()
...
```{r}
# SINTAXE DA FUNÇÃO LM
# modelo_ajustado = lm(variavel_resposta ~ variavel_preditora)
# modelo_ajustado = lm(variavel_resposta ~ preditores)  
  
fit <- lm(Height ~ HandSpan, data = dados)
names(fit)
