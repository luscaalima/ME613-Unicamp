...
```{r mostrar_codigo, eval=FALSE, echo=FALSE}
ggsave("matriz_dispersao.png", width = 12, height = 10, dpi = 300)

# ggsave("imagem_dispersao.png", width = 12, height = 10, dpi = 300)
...
corX1_X2 = round(cor(dados_continuos$IDHComp,dados_continuos$School),2)
```
A correlacao entre School e IDHComp e **`r corX1_X2`**  


```{r  echo=FALSE}
grafico1 = ggplot(dados, aes(y = LifeExp, x = School)) +
  geom_point(aes(color = School), size = 3, alpha = 0.6) +
  scale_color_gradientn(colors = cores_base) +
  theme_bw()+ theme(plot.title = element_text(face = "bold", hjust = 0.5))


grafico2 =ggplot(dados, aes(y = LifeExp, x = IDHComp)) +
  geom_point(aes(color = IDHComp), size = 3, alpha = 0.6) +
  scale_color_gradientn(colors = cores_base) +
  theme_bw()+ theme(plot.title = element_text(face = "bold", hjust = 0.5))


# grid.arrange(grafico1, grafico2, ncol = 2)
# g <- grid.arrange(grafico1, grafico2, ncol = 2, widths = c(1, 1))
# ggsave("graficos_combinados.png", g, width = 14, height = 6, dpi = 300)
```


```{r  echo=FALSE}
grafico3 = ggplot(dados_continuos, aes(y = LifeExp, x = School)) +
  geom_point(aes(color = School), size = 3, alpha = 0.6) +
  scale_color_gradientn(colors = cores_base) +
  theme_bw()+ theme(plot.title = element_text(face = "bold", hjust = 0.5))


grafico4 =ggplot(dados_continuos, aes(y = LifeExp, x = IDHComp)) +
  geom_point(aes(color = IDHComp), size = 3, alpha = 0.6) +
  scale_color_gradientn(colors = cores_base) +
  theme_bw()+ theme(plot.title = element_text(face = "bold", hjust = 0.5))
 ...
 # g1 <- grid.arrange(grafico3, grafico4, ncol = 2, widths = c(1, 1))
 # ggsave("graficos_combinados22.png", g1, width = 14, height = 6, dpi = 300)
```

```{r exibir_graficos_combinadados, echo=FALSE, out.width="90%", fig.align='center'}
# GRAFICOS COM VALORES = 0
knitr::include_graphics("graficos_combinados.png")
```
...
dados %<>% 
  ...
  filter(IDHComp > 0)
```
```{r exibir_graficos_combinadados2, echo=FALSE, out.width="90%", fig.align='center'}
# GRAFICOS COM VALORES != 0
knitr::include_graphics("graficos_combinados22.png")
```
