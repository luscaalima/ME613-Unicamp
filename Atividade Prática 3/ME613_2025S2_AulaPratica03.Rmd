...
# dados_continuos =  dados %>% 
#   select(-Developed) %>%
#    filter(IDHComp > 0 & School > 0)
...
# dados_continuos %>%  filter(IDHComp == 0)
...
```{r}
# modeloX2
# # Resíduos de Y dados X2: e_{y|x2} 
# # Extrair resíduos (parte de Y não explicada por X2)
# 
# dados$e_y_x2 <- resid(modeloX2)
```
<!-- Para remover o efeito de \( X_2 = IDHComp \) da variável resposta \( Y = LifeExp \), ajustou-se uma regressão simples de \( Y \) em \( X_2 \) e foram extraídos os resíduos \( e_{y|x2} \), que representam a parte da expectativa de vida não explicada pelo IDH.   -->
